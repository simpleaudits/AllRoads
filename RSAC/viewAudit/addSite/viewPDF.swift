    ...
    @objc func createPDFData(){
        ...
        case 1:
            ...
            for x in self.saveData{
                ...
                //Create a colums to insert data
                var tableOfUserData = PDFTable(rows: 6, columns: 3)

                //merge for image
                let row1 = tableOfUserData[rows:1...3, column: 0]
                row1.merge()
                // merge for comments
                let row2 = tableOfUserData[rows:4, columns: 0...2]
                row2.merge()
                
                let row3 = tableOfUserData[rows:5, columns: 0...2]
                row3.merge()
                
                
                let tL = PDFLineStyle(type: .full, color: UIColor.black, width: 2)
                ...
                tableOfUserData.content = [
                    ...
                    [nil, "Site name:",      "\(x.title)"],
                    [nil, "Date:",      "\(x.date)"],
                    [x.imageData, "Coordinate:",      "\(x.lat), \(x.long)"],
                    [nil, nil,     "Observation:"],
                    [ nil, nil,      "\(x.body)"],
    ...
    func loadSiteAuditData(){
                    ...
                    let group = DispatchGroup()
                        ...
                        for x in listOfSitesData{
                            ...
                            asyncData(imageURL: x.imageURL, descriptionData: x.auditDescription, auditTitleData: x.auditTitle, dateData: x.date, lat: "\(x.lat)",long: "\(x.long)") { result in do { group.leave()}
...
//                    }
    ...
    func asyncData(imageURL:String, descriptionData: String, auditTitleData:String, dateData:String,lat:String,long:String, completion: @escaping (String) -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            let task = URLSession.shared.dataTask(with: url!) {(data, response, error) in
                ...
                let pdfCreator = PDFCreatorData(title: auditTitleData, description: descriptionData, imageData: self.imageData, image: "", date: dateData, lat: lat, long: long)
                self.saveData.append(pdfCreator)
